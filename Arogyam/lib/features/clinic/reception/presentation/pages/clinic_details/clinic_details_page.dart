            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Clinic Operational Status (Open / Closed) Toggle
                  Container(
                    padding: const EdgeInsets.all(16),
                    decoration: BoxDecoration(
                      color: Colors.white,
                      borderRadius: BorderRadius.circular(16),
                      border: Border.all(
                        color: (clinic?.isOpen ?? true) ? const Color(0xFFBBF7D0) : const Color(0xFFFECACA),
                        width: 1.5,
                      ),
                    ),
                    child: Row(
                      children: [
                        Container(
                          padding: const EdgeInsets.all(10),
                          decoration: BoxDecoration(
                            color: (clinic?.isOpen ?? true) ? const Color(0xFFF0FDF4) : const Color(0xFFFEF2F2),
                            borderRadius: BorderRadius.circular(12),
                          ),
                          child: Icon(
                            (clinic?.isOpen ?? true) ? LucideIcons.doorOpen : LucideIcons.doorClosed,
                            color: (clinic?.isOpen ?? true) ? const Color(0xFF16A34A) : const Color(0xFFDC2626),
                            size: 22,
                          ),
                        ),
                        const SizedBox(width: 12),
                            ...
                            children: [
                              Row(
                                children: [
                                  ...
                                  Text(
                                    (clinic?.isOpen ?? true) ? 'OPEN' : 'CLOSED',
                                    style: GoogleFonts.inter(
                                      fontSize: 14,
                                      fontWeight: FontWeight.bold,
                              ...
                              Text(
                                (clinic?.isOpen ?? true)
                                    ? 'Patients can discover clinic, join queue & book visits'
                                    : 'Clinic is closed. Queue and bookings are disabled',
                                style: GoogleFonts.inter(
                                  fontSize: 11.5,
                                  color: AppColors.textSecondary,
                                ),
                        ...
                        Switch.adaptive(
                          value: clinic?.isOpen ?? true,
                          activeTrackColor: AppColors.teal,
                          onChanged: provider.isSaving
                              ? null
                              : (val) async {
                                  final success = await provider.toggleClinicOpen(val);
                                  if (!context.mounted) return;
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text(
                                        success
                                            ? 'Clinic marked as ${val ? 'OPEN' : 'CLOSED'}'
                                            : 'Failed to update clinic operational status',
                                      ),
                                      backgroundColor: success ? AppColors.teal : Colors.redAccent,
